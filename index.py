...
# list is sorted or not DISCENDING ORDER
lst = [7,6,5,4,2,1]
def is_sorted_des(user_list):
...
print(is_sorted_des(lst))    

# 2.if list in ascending or descending print True
def ascen_desce(user_list1,user_list2):
    if (is_sorted_asc(user_list1) == True) and (is_sorted_des(user_list2) == True):
        return True
    else:
        return False
        
print(ascen_desce(list1,lst)) 

#  List=[5,10,1,25,17,20]
# Print the prime numbers which are not presented in list range from min to max numbers in list
# Result=[2,3,7,11,13,17,19,23]
def prime(num):
    count = 0
    if num == 0 or num == 1:
        return False
...
for i in range(min1,max1+1):
    prime_res = prime(i)
    if prime_res == True and i not in list1:
        list2.append(i)
print(list2)  


list1 = [213,45,678,234,987]
# res:list2 = [False,True,True,True,False]
